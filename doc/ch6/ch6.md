...
* 예제
  ...
  * 위의 int recursiveSum(int n) 함수에서 재귀함수의 구성요소 3가지를 찾아보면 아래와 같습니다.
    ...
    * 조각마다 반복하기 : 조각 중 하나를 수행하고 나머지는 다시 자기자신에게 전달해서 수행하도록 합니다. 반복수행의 모습은 n + (n-1 + ... + 1) 형태가 되어야 합니다. 그래야 다음 호출에서도 처음과 똑같은 모습 n -1 + (n-2 + ... +  1) 을 유지할 수 있습니다. 
...
* 조각마다 반복하기
  ...
  * 현재 위치가 "G"라면 다음에는 "IRL"만 찾으면 됩니다.
  * 즉, 현재 위치의 8개 방향에 "IRL" 있는지 재귀함수를 반복할 수 있습니다.
  * 현재 위치를 (y, x) / 단어를 word 라고 해보겠습니다.
* 기저 사례
  * (지금까지 해왔던 방식의) 기저사례
    1. 현재 위치 (y, x) 가 word 의 첫 글자와 맞지 않는다면 실패입니다.
    2. (1번이 아니고) word가 한 글자라면 성공입니다.
  * 위의 내용과 더불어서 기저사례를 만들 때는 잘못된 입력에 대해서도 예외처리를 해주면 좋습니다.
    * 보글 게임은 주어진 격자가 있기 때문에 범위를 벗어나는 경우가 있을 수 있습니다.
* 구현
  ```c++
  ...
  * 코드분석
    ...
    * 그래서 가능한 경우를 하나라도 찾으면 true 가 연쇄적으로 반환되는 호출구조입니다.
...
* 그렇긴 하지만, 보글 게임에서 사용한 알고리즘은 해답을 찾으면 바로 리턴하기 때문에 약간 까다롭습니다.
* 최악의 경우
...
* 그럼 보글문제에서 문제와 부분문제는 어떻게 될까요?
  ...
  * 문제를 해결하기 위해서 9가지의 정보를 알아야 합니다.
    ...
    4. ...
    5. ...
  * 위의 9가지 정보 중에서 2~9번은 원래 문제의 응용버전입니다. 이들을 가리켜 부분문제라고 하며 재귀호출에서 조각으로 쓰여지는 것들입니다.
...
* 두 번째 테스트케이스
  ...
  * 4명을 보라돌이,뚜비,나나,뽀라고 할 때, 최종적으로 아래와 같은 3가지 방법이 가능함을 알 수 있습니다
    ...
    * (뚜비, 뽀) (보라돌이, 나나)
    * (나나, 뚜비) (뽀, 보라돌이)
  * 왜 3가지 방법일까요? (뒤에 계산공식이 나오긴 하지만 미리 설명합니다.)
